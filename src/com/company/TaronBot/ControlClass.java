...
public class ControlClass {
	...
	/**
	 * TODO read in networks mutate a list of networks adjust verbosity of output active Human game out puts weight generation data
	 * <p>
	 * generate a series of networks
	 * <p>
	 * generate -name/-n <default>required</default> -s/-size <size>default 5</size> -d/-depth <depth>default 8 </depth> -q/-quantity/-num <quantity>default 64</quantity> -complexity/ -c <complexity> default size*4</complexity> -seed <seed>recorded in log default no seed</seed>
	 * <p>
	 * select mutation method select mutation percentage help files log into playtak -name -password automate games on playtak
	 */

	/**
	 * This begins the Remote Control Structure: Reduced functionality to prevent harm to computer because my secruity is low stakbot ping: Sends a ping message to chat online: Begins generating AI(see generate for AI specs), Brings up Demo games(vs self) for playTak, Brings up sTAKbot for challenge on Playtak cleanup: Brings the AI down from online play. does not stop generation pause: pauses offline generation unpause: unpuases offline generation depth i: sets the depth of the networks generated with i rungens i: sets the gens to be run to i size i: sets the size of the board to be run gensize i: sets the size of the generation to i:
	 *
	 * @param r:
	 *            Scanner of the output from the site
	 * @param out:
	 *            output to the site
	 */
	...
	/**
	 * This begins the Local Control Structure enable/disable: load: Loads game from last run moves: shows moves made--Testing board: shows the top level boardstate each turn--Testing online: Begins generating AI(see generate for AI specs), Brings up Demo games(vs self) for playTak, Brings up sTAKbot for challenge on Playtak cleanup: Brings the AI down from online play. does not stop generation generate: Begins AI generation with rungens generations of size gensize with thread count of cores and Edge length of Board continue: Enables load and generates(see generate) then disables load after load complete. pause: pauses offline generation unpause: unpuases offline generation save: saves networks currently being run, halts generation depth i: sets the depth of the networks generated with i
	 * rungens i: sets the gens to be run to i size i: sets the size of the board to be run gensize i: sets the size of the generation to i: core i: sets the numbers of threads to run(defaults to 8) player1 i: sets the player 1 for local test game(from saved networks) player2 i: sets the player 2 for local test game(from saved networks) play: plays a game using player 1 and 2 with size size: Will fail if no saved network of size size with the player id
	 */
		...
		while (true) {
			...
			case "test":
				...
				Thread NEAT = new Thread() {
					@Override
					public void run() {
						RunGames.poolGamePlay(listOfItems, 5, 6000000, "stats", listOfItems.size());
					}
				};
				NEAT.start();
				break;
	...
	public static int generateCommand(Map<String, Object> objects, int size, int genSize, int runGens, int coreCount, Scanner reader) {
		...
		String name = "";
		/*
		 * -name/-n <default>required</default>a -s/-size <size>default 5</size> -d/-depth <depth>default 8 </depth> -q/-quantity/-num <quantity>default 64</quantity> -complexity/ -c <complexity> default size*4</complexity> -seed <seed>recorded in log default no seed</seed>
		 */
	...
	private static void GenerateNetworksOnline(int generationSize, int generations, int cores, int dimns) {
		...
		try {
			for (int i = 0; i < generations; i++) {
				...
				if (w != null) {
					w.write(StaticGlobals.roadCount + "," + StaticGlobals.gameCount + "," + StaticGlobals.moveCount + "," + StaticGlobals.flatCount + "," + StaticGlobals.wallCount + "," + StaticGlobals.capCount + "," + StaticGlobals.destackCount + "\n");
					w.flush();
				}
