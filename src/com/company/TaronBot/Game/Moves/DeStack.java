...
/**
 * Created by sarnowskit on 10/28/2016. A Destack is a Movement Operation. this involves direction, and the number left behind at each location in that direction
 */
public class DeStack implements Move {
	 ...
	 *            y cord
	 * @param left
	 *            Pieces left on position(Does not include starting position)
	 * @param pickup
	 *            Number of pieces to be picked up: Must agree with left
	 * @param verticalAxis
	 *            does the move go along the vertical acces
	 * @param positiveDirection
	 *            does the move go in the positive direction
	 * @param weight
	 *            what is the weight of the move(for sorting and AI usage)
	 * @return
	 * @throws Exception
	 */
	 ...
	 *
	 * @param map
	 *            represents whether the next piece is left on the same square or moves to the next square--includes starting position
	 * @param verticalAxis
	 ...
	 *            starting y
	 * @param left
	 *            left behind on each square does not include starting square
	 * @param pickup
	 *            pickup count must agree with left
	 * @param verticalAxis
	 *            along the vertical
	 * @param positiveDirection
	 *            in the posotive
	 * @param weight
	 *            eight of the move--use zero if not AI
	 */
	...
	/**
	 * Turns the move into a playtak.com string(the format for playtak to read it) "M " + sq1+" " +sq2+" " +left+ " " +left + ect. sq1= starting square in format s=letter(x), q=number(y+1) sq2= ending position se sq1
	 *
	 * @return
	 */
	...
	/**
	 * turns the move to a human readable string M + sq + direction(><+-) + left + left + ...
	 *
	 * @return
	 */
	@Override
	...
	public List<Integer>[][] performMove(Board board, boolean control) {
		...
		for (int i = 1; i <= leftBehind.size(); i++) {
			...
			if (!map[x + i * xChange * sign][y + i * yChange * sign].isEmpty() && Math.abs(map[x + i * xChange * sign][y + i * yChange * sign].get(map[x + i * xChange * sign][y + i * yChange * sign].size() - 1)) == 2) {
				map[x + i * xChange * sign][y + i * yChange * sign].set(map[x + i * xChange * sign][y + i * yChange * sign].size() - 1, map[x + i * xChange * sign][y + i * yChange * sign].get(map[x + i * xChange * sign][y + i * yChange * sign].size() - 1) / 2);
			}
	...
	private boolean capCheck(Board b) {
		...
		if (!map[xend][yend].isEmpty() && ((Math.abs(map[xend][yend].get(map[xend][yend].size() - 1)) == 2 && leftBehind.get(distance) != 1) || Math.abs(map[xend][yend].get(map[xend][yend].size() - 1)) == 3)) {
			return false;
		}
		int sign = -1;
		if (positive) {
