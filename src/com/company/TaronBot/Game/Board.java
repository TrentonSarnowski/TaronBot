...
/**
 * TakNetwork Created by sarnowskit on 10/21/2016.
 */
public class Board {
	 ...
	 * @param Player2
	 * @param Operator
	 *            used to do actions for every move
	 * @return
	 */
	public static int playGame(TakNetwork Player1, TakNetwork Player2, PerformOnMove Operator) {
		...
		game: while (!game.checkGameOver() && moveMade && turnCount < 1000 && !(m1.looped(m2) && n1.looped(n2))) {
			moveMade = false;
	 ...
	 *            board size
	 * @param boardState
	 *            moves that lead to the current board state
	 * @param start
	 *            who started the game(AI or not)
	 * @param boardID
	 *            ID of the board on playtak
	 * @return
	 */
	 ...
	 * 
	 * @param cont
	 *            who moved last to calculate ties
	 * @return
	 */
	public int checkVictory(boolean cont) {
		if (FastRoadFinder.RoadChecker(topLevelBoard(!cont)))
			return -32;
	 ...
	 * 
	 * @param b
	 *            weather the first player was in control or not
